...
// Morning Stretching + Meditation Tracker — React port
// Monochrome aesthetic to match ImmersionTracker. Single-file component with localStorage persistence.
// Features: per-day checkboxes, configurable minutes, inline timers, notes, streaks, week/month %, milestones, 6-week heatmap, JSON import/export.
...
const NumberInput = ({ value, onChange, min = 1, max = 60 }) => (
  <input type="number" className="w-20 rounded-lg border border-white/10 bg-neutral-900/70 px-3 py-2 text-neutral-100 focus:outline-none focus:ring-2 focus:ring-white/20" value={value} min={min} max={max} step={1} onChange={(e) => onChange(Math.max(min, Math.min(max, parseInt(e.target.value || "0", 10))))} />
);
...
const bestStreak = (state) => {
  ...
  for (const k of keys) {
    ...
    if (prev && prev === prevNextISO) cur += 1;
    else cur = 1;
          ...
          <div className="lg:col-span-3">
            <Card>
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <span className="pill rounded-full border border-dashed border-neutral-700 px-3 py-1 text-xs text-neutral-400">Single-file app</span>
              </div>
